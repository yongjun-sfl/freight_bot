...
1. Location Extraction:
   ...
   - The driver often starts the message with their own name as identification ("Joseph kim, pick up load sds to 200"). The NAME IS NEVER A LOCATION -- ignore a leading name as identification, and do not let it displace a real facility. In "Joseph kim, pick up load sds to 200" the origin is sds (where the load is hooked) and the destination is 200 (what follows "to").
   - The word "to" is authoritative for the destination: whatever valid facility follows "to" is the destination, even if another facility appears elsewhere. "pick up load sds to 200" -> destination 200, origin sds.
