        ...
        if is_arrival:
            ...
            async with p.acquire() as conn:
                async with conn.cursor() as cur:
                    ...
                    if row:
                        rid = row[0]
                        await cur.execute("UPDATE messages SET arrival_time=%s, status='arrived' WHERE id=%s", (telegram_time_str, rid))
        ...
        async with p.acquire() as conn:
            async with conn.cursor() as cur:
                await cur.execute(
                    ...
                    (sid, raw_text, parsed.get("origin"), parsed.get("destination"), telegram_time_str)
                )
                rid = cur.lastrowid
        ...
        async with p.acquire() as conn:
            async with conn.cursor() as cur:
                await cur.execute(
                    ...
                    (int(did), txt, parsed.get("origin"), parsed.get("destination"), telegram_time_str)
                )
                rid = cur.lastrowid
...
async def handle_photo_upload(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    logger.info(f"⚡ Processing photo ({len(img)} bytes) via Gemini Vision executor...")
        ...
        async with conn.cursor() as cur:
            ...
            if not row:
                ...
                await cur.execute(
                    ...
                    (did, img, b_num, t_num, ai_shipper, ai_receiver, status, telegram_time_str)
                )
                await conn.commit()
                rid = cur.lastrowid
